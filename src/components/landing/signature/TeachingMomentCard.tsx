...
export function TeachingMomentCard({ teacherAction = "Explaining concept", boardAction = "Writing equation", learnerResponse = "Asked clarifying question", nextDecision = "Replay example", className = "" }: TeachingMomentCardProps) {
  const steps = [
    { label: "Teacher", value: teacherAction, color: "text-[#7B1E2B]" },
    { label: "Board", value: boardAction, color: "text-[#A0522D]" },
    { label: "Learner", value: learnerResponse, color: "text-[#2E7D32]" },
    { label: "Next", value: nextDecision, color: "text-[#D97706]" },
  ];
  return (
    <div className={cn("bg-white rounded-xl border border-[#DDD5D0] p-5 shadow-sm", className)}>
      {steps.map((step, i) => (
        <div key={i} className={cn(i < steps.length - 1 && "border-b border-[#F2E8E1] pb-3 mb-3")}>
          <p className="text-[10px] font-bold uppercase tracking-wider text-[#7A7470] mb-1">{step.label}</p>
