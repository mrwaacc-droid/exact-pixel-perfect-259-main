...
export function LearningEvidenceTile({ question = "Why does carbon form 4 bonds?", explanation = "Adapted to use real-world analogy", example = "Replayed with visual diagram", completed = "Concept mastered", className = "" }: LearningEvidenceTileProps) {
  ...
  return (
    <div className={cn("bg-[#F4ECE4] rounded-xl border border-[#E7DAD1] p-4", className)}>
      {items.map((item, i) => (
        <div key={i} className="flex items-start gap-3">
          <div className="w-7 h-7 rounded-full bg-[#7D2233] text-white flex items-center justify-center text-[12px] font-bold shrink-0">{item.icon}</div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#8A7478]">{item.label}</p>
